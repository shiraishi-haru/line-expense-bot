...
- **スプレッドシートに反映されない**  
  1. **共有設定**  
     スプレッドシートの「共有」で、**サービスアカウントのメールアドレス**を**編集者**として追加してください。メールは `config/credential.json` の `client_email` に書かれています（例：`xxx@xxx.iam.gserviceaccount.com`）。共有していないと書き込みでエラーになります。  
  2. **ログで原因を確認**  
     ターミナル（または Render の Logs）に「スプレッドシート: 認証が未設定です」「スプレッドシート同期に失敗しました」などのメッセージが出ていないか確認してください。エラー内容が表示されるようになっています。  
  3. **Render で動かしている場合**  
     認証ファイルはデプロイに含まれないため、環境変数 **GOOGLE_CREDENTIALS_JSON** に、credential.json の内容を**1行にまとめた文字列**で設定してください。詳しくは **DEPLOY.md** の「Google スプレッドシート連携」を参照してください。
