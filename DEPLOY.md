...
**なぜ Render を推奨するか**  
このBotのような小規模サービスでは、**Render** の方が向いています。

- **無料で始められる** … クレジットカード不要で Web サービス＋PostgreSQL を試せます（無料枠の制限あり）。
- **GitHub 連携が簡単** … リポジトリを連携するだけでデプロイでき、`render.yaml` で一括設定も可能です。
- **Heroku との比較** … Heroku は 2022年11月以降、無料プランがなく月額約 $10〜 かかります。まずは無料で試したい場合は Render が適しています。長期・安定運用で有料でもよい場合は Heroku も選択肢です。

※ Render の無料 PostgreSQL は **1アカウントあたり1個まで** です。すでに別の無料DBを使っている場合は「cannot have more than one active free tier database」となるため、既存のDBをこのBot用に流用するか、SQLite で運用するか、有料 Postgres を追加してください。また、無料 Postgres は **約30日で期限** があるため、長期運用時は有料プランへのアップグレードが必要です。
...
`.env` や仮想環境、DBファイルをリポジトリに含めないため、プロジェクトルートに **.gitignore** があります。次のような内容になっていることを確認してください。
...
Render にデプロイする前に、同じコードがローカルで問題なく動くことを確認しておくと安心です。詳しいセットアップは **SETUP.md** を参照してください。
...
ここまで問題なければ、同じコードを Render にデプロイして本番運用に進めます。

---

## 4. Render で PostgreSQL を作成

※ **無料の PostgreSQL は1アカウントに1個まで** です。すでに別の無料DBがある場合は、そのDBをこのBot用に流用するか、有料 Postgres を追加してください。
...
※ Internal は同じ Render 内の Web サービスから接続する用です。
...
4. **Advanced** で **Add Environment Variable** を押し、次の環境変数を追加します。
   ...
   ⚠️ **重要**: `DATABASE_URL` には **PostgreSQL の接続文字列**（`postgresql://` で始まる）だけを設定してください。Web サービスの URL（`https://...onrender.com`）を入れると起動時にエラーになります。Render の PostgreSQL サービス → **Connections** → **Internal Database URL** をコピーして貼り付けてください。
   | `DEFAULT_INVITE_CODE` | 本番用招待コード（任意） |

   Google スプレッドシート連携をする場合のみ追加：

   | Key | Value |
   |-----|--------|
   | `GOOGLE_SPREADSHEET_ID` | スプレッドシートID（URL の `/d/` と `/edit` の間） |
   | `GOOGLE_CREDENTIALS_JSON` または `GOOGLE_CREDENTIALS_JSON_PATH` | 下記のどちらか |

   **Render では認証ファイルを置けないため、次のいずれかで設定します。**

   - **推奨: 環境変数 `GOOGLE_CREDENTIALS_JSON`**  
     サービスアカウントの JSON キーを**1行にまとめた文字列**でそのまま貼り付けます。  
     （JSON 内の改行を削除し、1行にしてから Environment の Value に貼り付け）
   - Secret File でファイルを配置した場合: `GOOGLE_CREDENTIALS_JSON_PATH` にそのファイルのパスを指定します。

   スプレッドシートの「共有」で、サービスアカウントのメールアドレス（`xxx@xxx.iam.gserviceaccount.com`）を**編集者**として追加してください。反映されない場合は Render のログで「スプレッドシート: 認証が未設定です」等のメッセージが出ていないか確認してください。

5. **Create Web Service** で作成します。
6. ビルドが成功すると、**https://xxxx.onrender.com** のような URL が発行されます。
...
**あなたがすることはありません。**  
Render の Web サービスは起動時に `init_db()` を実行し、テーブルを自動作成します（`app/main.py` の lifespan で実行）。初回デプロイでサービスが起動すれば、その時点でテーブルは作成されています。

招待コードを DB で管理しない場合は、環境変数 `DEFAULT_INVITE_CODE` を設定しておけば、そのコードでユーザーが登録できます。追加の作業は不要です。
...
3. 環境変数（`LINE_CHANNEL_SECRET` など）は **Dashboard の該当サービス** で手動設定してください。`sync: false` は「シークレットは手動で設定」の意味です。
4. データベースの **Internal Database URL** を、Web サービスの `DATABASE_URL` に設定します（Blueprint でリンクされる場合もあります）。

※ 無料DBを既に1つ持っている場合、Blueprint で新規に無料 Postgres を作ると「cannot have more than one active free tier database」になることがあります。その場合は手順4・5のように、手動で Web サービスだけ作成し、既存の DB の Internal URL を `DATABASE_URL` に設定してください。
...
| `GOOGLE_CREDENTIALS_JSON` | - | スプレッドシート用・サービスアカウントJSONの**文字列**（Render でファイルを置けない場合に使用） |
| `GOOGLE_CREDENTIALS_JSON_PATH` | - | スプレッドシート用・サービスアカウントJSONの**ファイルパス** |
| `GOOGLE_SPREADSHEET_ID` | - | 出力先スプレッドシートID |

---

以上で、Render へのデプロイと LINE Webhook の本番設定は完了です。
